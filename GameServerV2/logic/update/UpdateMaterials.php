...
class UpdateMaterials
{
    ...
    public function __construct($cityId, $finalTime = "NOW")
    {
        ...
        //do work
        $this->selectData();
        if($this->_finalTime >= $this->_initialTime){
            $this->calculateData();
            $this->updateData();
        }
    ...
    private function calculateData()
    {
        ...
        //Besieged material boxes loop they only produce the 25% but their maintenace cost is 50% that is added when a besiege is calculated
        ...
        /*//maintenances over 100 not needed
        ...
        $realMaintenanceCost = $this->_materialsInfo['buildings_maintenance'] + $this->_materialsInfo['troops_maintenance'] + $this->_materialsInfo['metal_maintenance_cost'] + $this->_materialsInfo['oil_maintenance_cost'] +  $this->_materialsInfo['gold_maintenance_cost'];

        /*//energies cost over 100 not needed
        ...
        $realEnergyCost = $this->_materialsInfo['energy_cost'] + $this->_materialsInfo['metal_energy_cost']	+ $this->_materialsInfo['oil_energy_cost'] +  $this->_materialsInfo['gold_energy_cost'];
        
        //low energy production correction
        if($hourProduction['ENERGY']<$realEnergyCost){
            $energyCoefficient = $hourProduction['ENERGY']/$realEnergyCost;
            $hourProduction['METAL'] *= $energyCoefficient;
            $hourProduction['OIL'] *= $energyCoefficient;
            $hourProduction['GOLD']  *= $energyCoefficient;
        }
