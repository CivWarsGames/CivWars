...
class Session
{
    ...
    public function __construct()
    {
        if(isset($_SESSION['userId'])){
            ...
            if(Parser::onlyNumbersString($_COOKIE['city'])){
                $selectUserCId = DataBaseManager::query("SELECT city_id FROM {buildings} WHERE city_id = $_COOKIE[city] AND
			     user_id = $_SESSION[userId]");
                if(DataBaseManager::numRows($selectUserCId)>0){
                    $selectUserCId = $_COOKIE['city'];
                }else{
            ...
            $user = new User($_SESSION['userId'], $selectUserCId);
            $this->user = $user;
        }else{
            require_once APP.'logic/tools/Parser.php';
            //Avoid SQL injection
    ...
    protected function login($name, $password, $serverId)
    {
        try
        {
            ...
            if(DataBaseManager::numRows($selectUserIds)>0){
                ...
                if(Parser::onlyNumbersString($_COOKIE['city'])){
                    $selectUserCId = DataBaseManager::query("SELECT city_id FROM {buildings} WHERE city_id = $_COOKIE[city] AND
                    user_id = $userIds[id_user]");
                    if(DataBaseManager::numRows($selectUserCId) > 0){
                        $userIds['current_city_id'] = $_COOKIE['city'];
                    }
                }
